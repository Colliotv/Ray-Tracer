...
** Last update Tue Jun  4 15:14:18 2013 vincent colliot
*/
...
void    cylindre_focus(void *l, t_token *token)
{
  ...
  if (!O_IN(s + strspn(s, "-+"), "0123456789."))
    def_error("x");
  ((t_cylindre*)l)->focus.x = (N(atof(s)) * M_PI) / 180;
  free(s);
  if ((s = xml_token(&token, "z", RESOLVE, 0)) == NULL)
    def_error("z");
  if (!O_IN(s + strspn(s, "-+"), "0123456789."))
    def_error("z");
  ((t_cylindre*)l)->focus.z = (N(atof(s)) * M_PI) / 180;
  free(s);
  if ((s = xml_token(&token, "y", RESOLVE, 0)) == NULL)
    def_error("y");
  if (!O_IN(s + strspn(s, "-+"), "0123456789."))
    def_error("y");
  ((t_cylindre*)l)->focus.y = (N(atof(s)) * M_PI) / 180;
  free(s);
